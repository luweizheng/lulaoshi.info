...
线性回归的一大假设是：误差服从均值为0的正态分布，且多个观测数据之间互不影响，相互独立。正态分布（高斯分布）的概率密度公式如下面公式，根据正太分布的公式，可以得到$\epsilon$的概率密度。

假设$x$服从正太分布，它的均值为$\mu$，方差为$\sigma$，它的概率密度公式如下。公式左侧的$P(x ; \mu, \sigma)$表示$x$是随机变量，$;$分号强调$\mu$和$\sigma$不是随机变量，而是这个概率密度函数的参数。条件概率函数中使用的$|$竖线有明确的意义，$P(y|x)$表示给定$x$（Given $x$），$y$发生的概率（Probability of $y$）。
$$
